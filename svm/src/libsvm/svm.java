...
//		svm_print_string.print(s);
	...
	public static void svm_cross_validation_onebyone(svm_problem prob, svm_parameter param, int nr_fold, double[] target)
	{
		int i;
		int[] fold_start = new int[nr_fold+1];
		int l = prob.l;
		...
		for(i=0;i<nr_fold;i++)
		{
			...
			for(j=0;j<begin;j++)
			{
				...
				if(prob.y[perm[j]]==0){
					...
					sl++;
					subprob.y[k] = 1;
					sbx.add(prob.x[perm[j]]);
					sby.add(prob.y[perm[j]]);
				}
			...
			svm_model submodel = svm_train(subprob,param);
			svm_model submodel2 = svm_train(sub2prob,param);
			if(param.probability==1 &&
			   (param.svm_type == svm_parameter.C_SVC ||
			    param.svm_type == svm_parameter.NU_SVC))
			{
				...
				for(j=begin;j<end;j++){
					double r = svm_predict(submodel,prob.x[perm[j]]);
					if(r != 0){
						r = svm_predict(submodel2,prob.x[perm[j]]);
					}
					target[perm[j]] = r;
				}					
		}
